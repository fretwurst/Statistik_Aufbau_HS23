...
$s_{b_2}^2=\frac{s^2}{T} \frac{1 / V_2}{1-r_{23}^2} \quad$ and $\quad s_{b_3}^2=\frac{s^2}{T} \frac{1 / V_3}{1-r_{23}^2}$
...
& =\frac{(1 / T) \sum_{t=1}^T\left\{\left[V_3\left(X_{t 2}-\bar{X}_2\right)-C_{23}\left(X_{t 3}-\bar{X}_3\right)\right]\left(Y_t-\bar{Y}\right)\right\}}{D}
\end{aligned}
$$
where $D=V_2 V_3-C_{23}^2$. From the true model for $Y$ and from averaging the $Y_t$ over the sample, we know that
$$
\begin{aligned}
Y_t-\bar{Y} & =\beta_1+\beta_2 X_{t 2}+\beta_3 X_{t 3}+U_t-\left(\beta_1+\beta_2 \bar{X}_2+\beta_3 \bar{X}_3+\bar{U}\right) \\
& =\beta_2\left(X_{t 2}-\bar{X}_2\right)+\beta_3\left(X_{t 3}-\bar{X}_3\right)+\left(U_t-\bar{U}\right)
\end{aligned}
$$
where $\bar{U}$ is the mean of all error terms implicit in the sample. By substitution, we have
$$
\begin{aligned}
& b_2=\frac{1}{T D}\left\{\sum_{t=1}^T\left[V_3\left(X_{t 2}-\bar{X}_2\right)-C_{23}\left(X_{t 3}-\bar{X}_3\right)\right]\right. \\
&\left.\times\left[\beta_2\left(X_{t 2}-\bar{X}_2\right)+\beta_3\left(X_{t 3}-\bar{X}_3\right)+\left(U_t-\bar{U}\right)\right]\right\} \\
&=\frac{1}{D}\left\{\frac{\beta_2}{T} V_3 \sum\left(X_{t 2}-\bar{X}_2\right)^2-\frac{\beta_2}{T} C_{23} \sum\left(X_{t 3}-\bar{X}_3\right)\left(X_{t 2}-\bar{X}_2\right)\right. \\
&+\frac{\beta_3}{T} V_3 \sum\left(X_{t 2}-\bar{X}_2\right)\left(X_{t 3}-\bar{X}_3\right)-\frac{\beta_3}{T} C_{23} \sum\left(X_{t 3}-\bar{X}_3\right)^2 \\
&\left.+\frac{1}{T} \sum\left[V_3\left(X_{t 2}-\bar{X}_2\right)-C_{23}\left(X_{t 3}-\bar{X}_3\right)\right]\left(U_t-\bar{U}\right)\right\}
\end{aligned}
$$
The first summation can be written as $\beta_2 V_3(1 / T) \Sigma\left(X_{t 2}-\bar{X}_2\right)^2=\beta_2 V_3 V_2$.

$$
\begin{aligned}
& E\left(C_{2 U}\right)=\frac{1}{T} \sum\left(X_{t 2}-\bar{X}_2\right) E\left(U_t-\bar{U}\right)=\frac{1}{T} \sum\left(X_{t 2}-\bar{X}_2\right)\left(\bar{U}_t-\mu\right)=C_{2 \bar{U}_t} \\
& E\left(C_{3 U}\right)=\frac{1}{T} \sum\left(X_{t 3}-\bar{X}_3\right) E\left(U_t-\bar{U}\right)=\frac{1}{T} \sum\left(X_{t 3}-\bar{X}_3\right)\left(\bar{U}_t-\mu\right)=C_3 \bar{U}_t,
\end{aligned}
$$
...
$$
\begin{aligned}
& E\left(C_{2 U}\right)=E\left[\frac{1}{T} \sum_t\left(X_{t 2}-\bar{X}_2\right)\left(U_t-\bar{U}\right)\right] \\
& E\left(C_{3 U}\right)=E\left[\frac{1}{T} \sum_t\left(X_{t 3}-\bar{X}_3\right)\left(U_t-\bar{U}\right)\right]
\end{aligned}
$$
...
:::{.callout-note collapse="true"}
## IYI: Ableitung für **$E(U_t-\overline{U}) = 0.$**
...
$$
This expression will equal $\beta_1$ if two conditions hold. If $U_t$ is distributed independently of $X_2$ and $X_3$, then our estimates $b_2$ and $b_3$ are unbiased so that $E\left(\beta_2-b_2\right)=E\left(\beta_3-b_3\right)=0$. Secondly, the expected value of $\bar{U}$ must equal zero, $E(\bar{U})=0$. If both conditions hold, $E\left(b_1\right)=\beta_1$. Thus, for all estimated coefficients to be unbiased, we modify assumption A.3 to be $E\left(U_t\right)=0$. Note that this is a stronger assumption than is needed for unbiasedness of the slope coefficients. $^3$
Arithmetic Properties
Unfortunately, we cannot test either of these assumptions, $E(\bar{U})=0$ and $\sum\left(X_{t k}-\bar{X}_k\right)\left(U_t-\bar{U}\right)=0$, with the observed data. Intuitively, one would test these assumptions by examining the mean of the residuals from the estimated equations and the correlation between these residuals and $X$. Our intuition, however, is wrong in this case. The mean of the residuals is
$$
\frac{1}{T} \sum_{t=1}^T\left(Y_t-\hat{Y}_t\right)=\frac{1}{T} \sum\left(Y_t-b_1-b_2 X_{t 2}-b_3 X_{3 t}\right)
$$
However, this is simply $-1 / T$ times Eq. (2.11) which by construction equals zero. Thus the mean of the estimated residuals must be zero, whether or not the mean of the true errors is zero.

Since $\bar{e}=0$, the numerator of the correlation between the residuals and $X_2$ is
$$
\begin{aligned}
\frac{1}{T} \sum e_t\left(X_{t 2}-\bar{X}_2\right) & =\frac{1}{T} \sum\left(Y_t-\hat{Y}_t\right)\left(X_{t 2}-\bar{X}_2\right) \\
& =\frac{1}{T} \sum\left(Y_t-\hat{Y}_t\right) X_{t 2}-\frac{1}{T} \sum\left(Y_t-\hat{Y}_t\right) \bar{X}_2 \\
& =\frac{1}{T} \sum\left(Y_t-b_1-b_2 X_{t 2}-b_3 X_{t 3}\right)\left(X_{t 2}\right)-\frac{1}{T} \bar{X}_2 \sum e_t
\end{aligned}
$$
:::
...
$$
\begin{aligned}
& E\left(C_{2 U}^2\right)=E\left\{\frac{1}{T^2}\left[\sum\left(X_{t 2}-\bar{X}_2\right)\left(U_t-\bar{U}\right)\right]^2\right\} \\
&=\frac{1}{T^2} E {\left[\left(X_{12}-\bar{X}_2\right)\left(U_1-\bar{U}\right)+\left(X_{22}-\bar{X}_2\right)\left(U_2-\bar{U}\right)\right.} \\
&\left.+\cdots+\left(X_{T 2}-\bar{X}_2\right)\left(U_T-\bar{U}\right)\right]^2
\end{aligned}
$$
(from expanding the summation)
$$
\begin{aligned}
&=\frac{1}{T^2} E\left[\sum_{t=1}^T\left(X_{t 2}-\bar{X}_2\right)^2\left(U_t-\bar{U}\right)^2\right. \\
&\left.+2 \sum_{t=1}^{T-1} \sum_{s=t+1}^T\left(X_{t 2}-\bar{X}_2\right)\left(X_{s 2}-\bar{X}_2\right)\left(U_t-\bar{U}\right)\left(U_s-\bar{U}\right)\right]
\end{aligned}
$$

$$
\begin{aligned}
&=\frac{1}{T^2}[ \sum_{t=1}^T\left(X_{t 2}-\bar{X}_2\right)^2 E\left(U_t-\bar{U}\right)^2 \\
&\left.+2 \sum_{t=1}^T \sum_{s=t+1}^T\left(X_{t 2}-\bar{X}_2\right)\left(X_{s 2}-\bar{X}_2\right) E\left(U_t-\bar{U}\right)\left(U_s-\bar{U}\right)\right] \\
& \text { (from fixed } X \text { 's) } \\
&= \frac{1}{T^2}\left[\sum\left(X_{t 2}-\bar{X}_2\right)^2 \sigma_t^2+2 \sum \sum\left(X_{t 2}-\bar{X}_2\right)\left(X_{s 2}-\bar{X}_2\right) \sigma_{t s}\right], \text { (3.6) }
\end{aligned}
$$
where $\sigma_t^2=E\left(U_t-\bar{U}\right)^2$ and $\sigma_{t s}=E\left(U_t-\bar{U}\right)\left(U_s-\bar{U}\right)$. Similar expressions for $E\left(C_{2 U} C_{3 U}\right)$ and $E\left(C_{3 U}^2\right)$ are
$$
\begin{aligned}
& E\left(C_{3 U}^2\right)=\frac{1}{T^2}\left[\sum\left(X_{t 3}-\bar{X}_3\right)^2 \sigma_t^2+2 \sum \sum\left(X_{t 3}-\bar{X}_3\right)\left(X_{s 3}-\bar{X}_3\right) \sigma_{t s}\right], \\
& E\left(C_{2 U} C_{3 U}\right)=\frac{1}{T^2}\left[\sum\left(X_{t 2}-\bar{X}_2\right)\left(X_{t 3}-\bar{X}_3\right) \sigma_t^2\right. \\
& \left.+2 \sum \sum\left(X_{t 2}-\bar{X}_2\right)\left(X_{s 3}-\bar{X}_3\right) \sigma_{t s}\right] \text {. } \\
&
\end{aligned}
$$
...
A.4a
$$
E\left(U_t-\bar{U}\right)^2=\sigma_t^2=\sigma^2 \quad \text { for all } t .
$$
...
A.4b
$$
E\left(U_t-\bar{U}\right)\left(U_s-\bar{U}\right)=\sigma_{t s}=0 \quad \text { for } \quad t \neq s .
$$
With these two assumptions, or restrictions on the error terms, the above simplify to
$$
\begin{aligned}
& E\left(C_{2 U}^2\right)=\frac{1}{T^2} \sum\left(X_t-X_2\right)^2 \sigma^2=\frac{\sigma^2 V_2}{T}, \\
& E\left(C_{3 U}^2\right)=\frac{\sigma^2 V_3}{T},
\end{aligned}
$$
...
$$
\begin{aligned}
\operatorname{var}\left(b_2\right) & =\frac{\sigma^2}{T D^2}\left[V_3^2 V_2-2 V_3 C_{23}^2+C_{23}^2 V_3\right]=\frac{\sigma^2 V_3}{T D^2}\left[V_3 V_2-C_{23}^2\right] \\
& =\frac{\sigma^2 V_3}{T D}=\frac{1}{T} \sigma^2\left[\frac{V_3}{V_2 V_3-C_{23}^2}\right]=\frac{\sigma^2}{T}\left[\frac{1 / V_2}{1-r_{23}^2}\right] .
\end{aligned}
$$
The similar expression for $\operatorname{var}\left(b_3\right)$ is
$$
\operatorname{var}\left(b_3\right)=\frac{1}{T} \sigma^2\left[\frac{V_2}{V_2 V_3-C_{23}^2}\right]=\frac{\sigma^2}{T}\left[\frac{1 / V_3}{1-r_{23}^2}\right] .
$$
[A useful exercise for the student is to show for the bivariate case that $\left.\operatorname{var}\left(b_2\right)=\sigma^2 / \Sigma\left(X_t-\bar{X}\right)^2=\sigma^2 / T \operatorname{var}(X).\right]$
:::
...
The proof that $b_2$ is best is only sketched here. A complete proof is shown in Appendix 5.1. The proposition to be demonstrated is that, among all linear and unbiased estimators of $\beta_2$ and $\beta_3$, the least squares estimators $b_2$ and $b_3$ have the minimum variance when assumptions A.1-A.4 hold. We first define an arbitrary linear estimator $b_2^{\#}$. Linear refers to the fact that the estimator is a linear function of the $Y_t, b_2^{\#}=\Sigma C_{t 2}^{\#}\left(Y_t-\bar{Y}\right)$, where $C_{t 2}^{\#}$ is any set of weights. (The weights are
$$
C_{t 2}=\frac{1}{T} \frac{V_3\left(X_{t 2}-\bar{X}_2\right)-C_{23}\left(X_{t 3}-\bar{X}_3\right)}{D}
$$
for the least squares estimator of $\beta_2$.) With complete generality, we can write $C_{t 2}^{\#}$ as the least squares weight plus an arbitrary number $g_{t 2}, C_{t 2}^{\#}=C_{t 2}+g_{t 2}$. The restriction of unbiasedness implies that $E\left[\Sigma C_{t 2}^{\#}\left(Y_t-\bar{Y}\right)\right]=\beta_2$. However, for OLS we showed that $E\left[\Sigma C_{t 2}\left(Y_t-\bar{Y}\right)\right]=\beta_2$. This implies that $E\left[\Sigma g_{t 2}\left(Y_t-\right.\right.$ $\bar{Y})]=0$ since $E\left[\Sigma C_{t 2}^{\#}\left(Y_t-\bar{Y}\right)\right]=E\left[\Sigma C_{t 2}\left(Y_t-\bar{Y}\right)+\Sigma g_{t 2}\left(Y_t-\bar{Y}\right)\right]$. Using this restriction and assumption A.4, the variance of $b_2^{\#}$ is
$$
\operatorname{var}\left(b_2^{\#}\right)=\operatorname{var}\left(b_2\right)+\sigma^2 \sum_{t=1}^T g_{t 2}^2,
$$
where $\operatorname{var}\left(b_2\right)$ is the variance of the least squares estimator. Since $g_{t 2}^2 \geqslant 0$, $\operatorname{var}\left(b_2^{\#}\right)$ cannot be less than the variance of the least squares estimator $b_2$. Further, it can equal $\operatorname{var}\left(b_2\right)$ only if each perturbation $\left(g_{t 2}\right)$ from the least squares weight is identically zero. (Similar developments can be done for $b_1$ and $b_3$.)

An important aspect of this development, however, is that we have accepted and used the assumptions about the error distribution and fixed $X$ 's. That is, this proof holds when $E\left(U_t-\bar{U}\right)=0, E\left(U_t-\bar{U}\right)^2=\sigma^2$, and $E\left(U_t-\right.$ $\bar{U})\left(U_s-\bar{U}\right)=0$ for all $t$ and $s \neq t$.
:::
...
:::{.callout-note collapse="true"}
## IYI: Ableitung für $U_t \sim N\left(0, \sigma^2\right)$ .

If $U_t \sim N\left(0, \sigma^2\right)$, and independent of $U_s$, then the $b$ 's, which are linear functions of $U_t$, are normally distributed with a variance given in Eq. (3.5a), that is, $b_2 \sim N\left(\beta_2, \sigma^2 / T V_2\left(1-r_{23}^2\right)\right)$. This implies that
$$
...
$s^2$ is an unbiased estimator of $\sigma^2$ and is substituted for $\sigma$ in our expression for $\sigma_b$. However, any inferences employing $s^2$ will not be as precise as they would be if we knew $\sigma^2$ and did not have to rely upon the random variable $s^2$. In order to allow for this additional imprecision, we do not use probabilities from the normal distribution. Instead, we shall rely upon the $t$-distribution. The definition of the $t$-distribution is as follows: if $Z$ is a standard normal variable, i.e., $Z$ is $N(0,1)$, and if $W^2$ is an independently distributed chisquared with $T-3$ degrees of freedom, then $Z / \sqrt{W^2 /(T-3)}$ is distributed according to the $t$-distribution with $T-3$ degrees of freedom. We have demonstrated that $(b-\beta) / \sigma_b$ is $N(0,1)$. It can further be shown that (Hoel, 1962, pp. 262-268)
$$
W^2=\sum e_t^2 / \sigma^2 \quad \text { is } \quad \chi_{T-3}^2 .
$$
For $b_2$ we get
$$
\begin{aligned}
t_{b_2} & =\frac{Z}{\sqrt{W^2 /(T-3)}}=\frac{\left(b_2-\beta_2\right) / \sigma_{b_2}}{\sqrt{\sum e_t^2 / \sigma^2(T-3)}} \\
& =\frac{\left(b_2-\beta_2\right)}{(\sigma / T)\left[1 / V_2\left(1-r_{23}^2\right)\right]} \frac{1}{\sqrt{\sum e_t^2} / \sigma \sqrt{T-3}}=\frac{b_2-\beta_2}{(s / T)\left(\left(1 / V_2\right) /\left(1-r_{23}^2\right)\right)} \\
& =\frac{b_2-\beta_2}{s_{b_2}},
\end{aligned}
$$
where $s^2=\sum e_t^2 /(T-3)$. This variable $t_{b_2}$ is distributed as Student's $t$ with $T-3$ degrees of freedom.

$$
t_b=|b| / s_b>t_{\text {crit }(\alpha / 2, T-3)},
$$
$t_{\text {crit }(\alpha / 2, T-3)}$ is the critical value for $T-3$ degrees of freedon ificance level of $\alpha$. The significance level $\alpha$ is the size of a type I probability of rejecting the null hypothesis when it is in fact ole terms, the further $b$ is from zero (i.e., the higher $t_b$ ), the less lik $\beta$ is really zero. The general form of the hypothesis test for $H_0$ :
$$
t_b=\left|b-\beta^*\right| / s_b .
$$
$>t_{\text {crit }(\alpha / 2, T-3)}$, the null hypothesis is rejected. In other words
:::
...
```{=tex}
\begin{align}
  d =& \frac{\sum_{t=2}^T(e_t-e_{t-1})^2}{\sum_{t=1}^T e^2_t} \label{eq:DWT}
\end{align}
```
